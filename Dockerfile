...
ENV EXTERNAL_RTP_IP ${EXTERNAL_RTP_IP:-'$${local_ip_v4}'}
...
# Adjust RTP external_ip
#RUN sed -i /etc/freeswitch/vars.xml -e 's/<X-PRE-PROCESS cmd="set" data="external_rtp_ip=\$\${local_ip_v4}" \/>/<X-PRE-PROCESS cmd="set" data="external_rtp_ip="${EXTERNAL_RTP_IP}" \/>/g'
